...
// Compiled from: GOVERNING_PHYSICS.md, ENVIROSENSE_MASTER_ARCHITECTURE.md,
// DIGITAL_TWIN_2_0.md, TECHNICAL_DOSSIER.md
// ============================================================
...
function buildSensorSnapshot(ctx: SensorContext): string {
  const lines: string[] = [
...
export async function sendChatMessage(
  ...
  if (!apiKey) {
    return "⚠ AI service not configured. Set NEXT_PUBLIC_GEMINI_API_KEY in your .env.local file with a Google AI Studio API key.";
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return "Rate limit reached. The free tier has request limits — please wait a moment and try again.";
      }
      if (response.status === 400) {
        return "Request error. The model may not be available in your region. Check your API key configuration.";
      }
    ...
    if (!text) {
      console.error("[Gemma4] Unexpected response shape:", JSON.stringify(data));
      return "I received an empty response. This may be a temporary issue — please try again.";
    }

    return text;
  } catch (error: any) {
    console.error("[Gemma4] Network error:", error);
